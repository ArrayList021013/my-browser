import { IFormFillData } from "~/interfaces/form-fill";
...
export const getFormFillValue = (name: string, data: IFormFillData, hidePassword = false) => {
  ...
  try {
    switch (name) {
      ...
      default:
        //return fields?.[getKey(name) ?? 'unknown'] ?? 'password';
        const key = getKey(name);
        if (fields && key !== null && key !== undefined) {
          return fields[key];
        } else {
          throw new Error('Invalid fields or key');
        }
    }
  } catch (error) {}
...
export const getFormFillSubValue = (name: string, data: IFormFillData) => {
  ...
  if (data.type === 'address') {
    ...
    return '●'.repeat(fields?.passLength ?? 0);
  }
